...
def test_existing_annotation_statuses_are_consolidated_without_new_matching_rules():
    ...
    assert mod.status({"MTCODON_STATUS":"AMBIGUOUS_CODON"})[:2]==("CDS","AMBIGUOUS")
    assert mod.status({"MTCODON_STATUS":"SKIPPED_NONCODING","MTTRNA_STATUS":"OK","MTTRNA_STRICT_MATCH":"no"})[:2]==("tRNA","FAIL")
    assert mod.status({"MTCODON_STATUS":"SKIPPED_NONCODING","MTTRNA_STATUS":"NO_SPECIES_OR_HUMAN_TRNA","MTRRNA_STATUS":"NO_SPECIES_OR_HUMAN_RRNA"})[:2]==("noncoding","NOT_APPLICABLE")
...
''')
    ...
    assert (out/"primate_homo_background_metadata.json").is_file()
