...
def safe_hash(path):
    ...
    try: return normalized_fasta_sequence_sha256(path)
    except (OSError, ValueError): return None
def read_map(path):
    indexes = {'key': {}, 'hash': {}, 'accession': {}, 'species': {}}
    if not path or not Path(path).is_file(): return indexes
    with Path(path).open(newline='') as handle:
        for row in csv.DictReader(handle, delimiter='\t'):
            for name, field in (('key','reference_key'), ('hash','coordinate_reference_sequence_sha256'), ('accession','coordinate_reference_accession')):
                if v(row, field): indexes[name].setdefault(v(row, field), row)
            if v(row, 'species'): indexes['species'].setdefault(species_key(v(row, 'species')), row)
    return indexes
def configured_fasta(species, directory, extensions):
    if not species or not directory or not Path(directory).is_dir(): return ''
    suffixes=sorted((x.strip() for x in str(extensions).split(',') if x.strip()), key=len, reverse=True)
    for path in sorted(Path(directory).iterdir()):
        if path.is_file() and any(path.name.endswith(x) and species_key(path.name[:-len(x)]) == species_key(species) for x in suffixes): return str(path)
    return ''
def meta(rows, mitos=False, map_indexes=None, species_fasta_dir='', fasta_extensions='.fa,.fasta,.fna,.fa.gz,.fasta.gz,.fna.gz'):
    r=rows[0] if rows else {}; recorded=v(r,'coordinate_reference_fasta') or (v(r,'mitos2_input_fasta') if mitos else '')
    ...
    species=v(r,'reference_species') or v(r,'species') or v(r,'target_species'); target=v(r,'target_species') or v(r,'species')
    resolved=''; source=''; status=''; reason=''
    if h: source=status='existing_table_hash'
    else:
        indexes=map_indexes or {}; mapped=(indexes.get('key',{}).get(v(r,'reference_key')) or indexes.get('accession',{}).get(accession) or indexes.get('species',{}).get(species_key(target or species)))
        if mapped and v(mapped,'coordinate_reference_fasta'):
            info=safe_hash(v(mapped,'coordinate_reference_fasta'))
            if info: resolved=info['fasta_path']; h=info['sequence_sha256']; length=str(info['sequence_length']); source=status='sample_reference_map'
            else: reason='sample_reference_map_fasta_missing'
        if not h:
            candidate=configured_fasta(target or species, species_fasta_dir, fasta_extensions); info=safe_hash(candidate)
            if info: resolved=info['fasta_path']; h=info['sequence_sha256']; length=str(info['sequence_length']); source=status='configured_species_fasta_dir'
        if not h:
            info=safe_hash(recorded)
            if info: resolved=info['fasta_path']; h=info['sequence_sha256']; length=str(info['sequence_length']); source=status='recorded_table_path'
        if not h:
            source=status='unresolved'; reason=reason or ('recorded_coordinate_fasta_missing' if recorded else 'no_coordinate_fasta_or_hash')
    return {'key':v(r,'reference_key'),'path':resolved or recorded,'recorded_path':recorded,'resolved_path':resolved,'hash':h,'length':length,'fasta_resolution_status':status,'fasta_resolution_source':source,'missing_file_reason':reason,
            'accession':accession,'species':species,'target':target,'record_hash':v(r,'genbank_record_sequence_sha256'), 'input_hash':v(r,'mitos2_input_sequence_sha256') or h}
def rotation(a,b):
...
def atomic(path, fields, rows):
    ...
    os.replace(tmp,path)
def compare(genbank_table,mitos_table,output,summary_output,mismatch_output,strict=True,allow_rotation=False,fail_no_shared=True,minor=.99,moderate=.90, sample_reference_map=None, species_fasta_dir="", fasta_extensions=".fa,.fasta,.fna,.fa.gz,.fasta.gz,.fna.gz"):
    gb=read_groups(genbank_table); mi=read_groups(mitos_table); indexes=read_map(sample_reference_map); gm={k:meta(x,False,indexes,species_fasta_dir,fasta_extensions) for k,x in gb.items()}; mm={k:meta(x,True,indexes,species_fasta_dir,fasta_extensions) for k,x in mi.items()}
    recorded=[x for x in gm.values() if x['recorded_path']]
    existing=sum(Path(x['recorded_path']).is_file() for x in gm.values() if x['recorded_path'])
    remapped=sum(x['fasta_resolution_source'] in ('sample_reference_map', 'configured_species_fasta_dir') for x in gm.values())
    unresolved=sum(x['fasta_resolution_status']=='unresolved' for x in gm.values())
    print(f'Recorded coordinate FASTA paths: {len(recorded)}; existing: {existing}; stale/missing: {len(recorded)-existing}; successfully remapped: {remapped}; unresolved: {unresolved}')
    candidates=[]
    for g, x in gm.items():
        key_matches=[m for m in mm if m == g]
        possible=key_matches or [m for m,y in mm.items() if (x['hash'] and y['hash'] and x['hash']==y['hash']) or (x['accession'] and x['accession']==y['accession'])]
        for m in possible:
            y=mm[m]
            candidates.append((g,m,'sequence_sha256' if x['hash'] and y['hash'] and x['hash']==y['hash'] else 'reference_key_or_accession'))
    ...
    for g,m,basis in candidates:
        ...
        elif not x['hash'] and not y['hash']: cat='missing_coordinate_fasta' if (x['fasta_resolution_status']=='unresolved' or y['fasta_resolution_status']=='unresolved') else 'missing_both_sequence_hash'
        elif not x['hash']: cat='missing_coordinate_fasta' if x['fasta_resolution_status']=='unresolved' else 'missing_genbank_sequence_hash'
        elif not y['hash']: cat='missing_coordinate_fasta' if y['fasta_resolution_status']=='unresolved' else 'missing_mitos2_sequence_hash'
        ...
        base={'genbank_reference_key':g,'mitos2_reference_key':m,'target_species':x['target'] or y['target'],'reference_species':x['species'] or y['species'],'genbank_accession':x['accession'],'mitos2_accession':y['accession'],'genbank_coordinate_fasta':x['path'],'mitos2_coordinate_fasta':y['path'],'recorded_coordinate_reference_fasta':x['recorded_path'],'resolved_coordinate_reference_fasta':x['resolved_path'],'fasta_resolution_status':x['fasta_resolution_status'],'fasta_resolution_source':x['fasta_resolution_source'],'missing_file_reason':x['missing_file_reason'],'genbank_coordinate_sequence_length':x['length'],'mitos2_coordinate_sequence_length':y['length'],'genbank_coordinate_sequence_sha256':x['hash'],'mitos2_coordinate_sequence_sha256':y['hash'],'genbank_record_sequence_sha256':x['record_hash'],'mitos2_input_sequence_sha256':y['input_hash'],'sequence_compatibility_category':cat,'sequence_match':yes(cat in ('exact_sequence_match','coordinate_fasta_match')),'rotation_equivalent':rot,'rotation_offset':offset,'coordinate_comparison_performed':yes(perform)}
        if not perform:
    ...
    unresolved=sum(x['sequence_compatibility_category'].startswith('missing') for x in summaries)
    if fail_no_shared and (not candidates or not gene_rows):
        raise RuntimeError(f'No sequence-compatible reference pairs after diagnostics were written; {unresolved} unresolved references.')
