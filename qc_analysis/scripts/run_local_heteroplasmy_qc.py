...
def patterns(value, defaults: list[str]) -> list[str]:
    """Keep configured names but always append known production filename forms."""
...
def eligible_het(row: dict, sec: dict) -> bool:
    ...
    return (
        ...
        and dp is not None and dp > float(sec.get("dp_min", 100))
        and af is not None
...
def main() -> int:
    ...
    eligible = [dict(r) for r in raw_rows if eligible_het(r, sec)]
    for r in eligible:
    ...
    for s in eligible_samples:
        sample_to_species.setdefault(s, "")
    ...
    # Production files live together under data/numt_besthit and are named
    # SAMPLE.numt_vs_chrM.besthit.tsv and SAMPLE.highconf_numt.bed.
    ...
    besthit_dir = choose_numt_dir(sec.get("numt_besthit_dir"), besthit_pats, eligible_samples, [repo_numt_dir])
    highconf_dir = choose_numt_dir(sec.get("numt_highconf_dir"), highconf_pats, eligible_samples, [besthit_dir, repo_numt_dir])
    n_besthit_files = count_sample_files(besthit_dir, besthit_pats, eligible_samples)
    n_highconf_files = count_sample_files(highconf_dir, highconf_pats, eligible_samples)
    if eligible_samples and n_besthit_files == 0:
        raise RuntimeError(
            f"no NUMT besthit files found; checked {besthit_dir}; expected e.g. SAMPLE.numt_vs_chrM.besthit.tsv"
        )

    print(f"[local_heteroplasmy_qc] metadata species={len(eligible_samples)-len(missing_species)}/{len(eligible_samples)} path={metadata_path}", file=sys.stderr)
    print(f"[local_heteroplasmy_qc] besthit={n_besthit_files}/{len(eligible_samples)} dir={besthit_dir}", file=sys.stderr)
    print(f"[local_heteroplasmy_qc] highconf={n_highconf_files}/{len(eligible_samples)} dir={highconf_dir}", file=sys.stderr)
    ...
    for i, iv in enumerate(species_intervals, 1):
        ...
        species_rows.append({
            ...
            "species": iv["species"], "reference_key": iv["reference_key"], "tier": iv["tier"],
            "chrm_start": iv["chrm_start"], "chrm_end": iv["chrm_end"],
            "n_support_samples": len(support), "support_samples": ",".join(support),
            "n_nuclear_loci": len(loci), "nuclear_loci": ";".join(f"{c}:{s}-{e}" for c, s, e in loci),
        })
    write_tsv(report_dir / "species_numt_intervals.tsv", species_rows,
    ...
    for sample, srows in sorted(by_sample.items()):
        ...
        for cnum, indices in enumerate(expanded, 1):
            ...
            rec = assess_recurrence(
                members, sample, species, recurrence_index,
            ...
            if sample_ok:
                tier, ns, ne = interval.tier, interval.chrm_start, interval.chrm_end
                sup_n, sup_samples = 0, ""
            elif species_ok:
                tier, ns, ne = interval["tier"], interval["chrm_start"], interval["chrm_end"]
                sup_samples = ",".join(species_hit["other_support_samples"])
                sup_n = len(species_hit["other_support_samples"])
            else:
                tier, ns, ne, sup_n, sup_samples = "NONE", "NA", "NA", 0, ""

            crow = {
                "sample": sample, "species": species, "reference_key": refkey, "cluster_id": cid,
                "cluster_start": sm["cluster_start"], "cluster_end": sm["cluster_end"], "cluster_span_bp": sm["cluster_span_bp"], "wraps_origin": sm["wraps_origin"],
                "seed_n_variants": len(seeds[cnum-1]) if cnum-1 < len(seeds) else len(indices), "expanded_n_variants": sm["n_variants"],
                "median_af": f"{sm['median_af']:.6g}", "af_min": f"{sm['af_min']:.6g}", "af_max": f"{sm['af_max']:.6g}", "af_span": f"{sm['af_span']:.6g}",
                "observed_max": observed, "critical_count": crit, "empirical_p": "NA" if pval is None else f"{pval:.6g}",
                "sample_numt_evidence": "YES" if sample_ok else "NO", "species_numt_evidence": "YES" if species_ok else "NO", "any_numt_evidence": "YES" if any_numt else "NO",
                "numt_scope": scope, "numt_tier": tier, "numt_chrm_start": ns, "numt_chrm_end": ne,
                "numt_overlap_n": hit["overlap_n"] if hit else 0, "numt_overlap_fraction": f"{hit['overlap_fraction']:.6g}" if hit else "0",
                "species_numt_support_n": sup_n, "species_numt_support_samples": sup_samples,
                "recurrence": "YES" if recurrent else "NO", "best_recurrent_sample": rec["best_recurrent_sample"],
                "shared_variants": rec["shared_variants"], "shared_fraction": f"{rec['shared_fraction']:.6g}",
                "median_af_difference": rec["median_af_difference"] if rec["median_af_difference"] == "NA" else f"{rec['median_af_difference']:.6g}",
                "cluster_class": cls, "filter_action": action, "filter_reason": reason,
            }
            cluster_rows.append(crow); class_counts[cls] += 1; class_vars[cls] += len(indices); scount[cls] += 1
            for i in indices:
                ann[i] = {"cluster_id": cid, "cluster_class": cls, "filter_action": action, "filter_reason": reason, "numt_scope": scope, "numt_tier": tier, "recurrence": "YES" if recurrent else "NO"}
                if action == "REMOVE":
                    removed += 1
                    m = srows[i]
                    removal_rows.append({"sample": sample, "species": species, "source_chrom": m.get("source_chrom", ""), "source_pos": m["source_pos"], "source_ref": m.get("source_ref", ""), "source_alt": m.get("source_alt", ""), "source_af": f"{float(m['source_af']):.6g}", "source_dp": f"{float(m['source_dp']):.6g}", "cluster_id": cid, "cluster_class": cls, "numt_scope": scope, "numt_tier": tier, "filter_action": action, "filter_reason": reason})

        for i, m in enumerate(srows):
            a = ann[i]
            variant_rows.append({"sample": sample, "species": species, "source_chrom": m.get("source_chrom", ""), "source_pos": m["source_pos"], "source_ref": m.get("source_ref", ""), "source_alt": m.get("source_alt", ""), "source_af": f"{float(m['source_af']):.6g}", "source_dp": f"{float(m['source_dp']):.6g}", "clustered": "YES" if a["cluster_id"] else "NO", "cluster_id": a["cluster_id"], "cluster_class": a["cluster_class"], "numt_scope": a.get("numt_scope", "NONE"), "numt_tier": a.get("numt_tier", "NONE"), "recurrence": a.get("recurrence", "NO"), "filter_action": a["filter_action"], "filter_reason": a["filter_reason"]})

        ncl = len(expanded); nclustered = sum(map(len, expanded)); nnumt = scount["NUMT_RECURRENT"] + scount["NUMT_ONLY"]
        sr = {"sample": sample, "species": species, "n_het": len(srows), "assessed": str(assessed).lower(), "observed_max": observed, "critical_count": crit if assessed else "NA", "empirical_p": "NA" if pval is None else f"{pval:.6g}", "n_clusters": ncl, "n_clustered_het": nclustered, "n_numt_clusters": nnumt, "n_numt_recurrent_clusters": scount["NUMT_RECURRENT"], "n_numt_only_clusters": scount["NUMT_ONLY"], "n_recurrent_only_clusters": scount["RECURRENT_ONLY"], "n_unresolved_clusters": scount["UNRESOLVED"], "n_numt_variants_to_remove": removed, "fraction_het_removed": f"{removed/len(srows):.6g}" if srows else "0", "numt_sample": "YES" if nnumt else "NO", "sample_filter_action": "KEEP"}
        sample_rows.append(sr)
        if nnumt: numt_sample_rows.append(dict(sr))

    cluster_fields = ["sample","species","reference_key","cluster_id","cluster_start","cluster_end","cluster_span_bp","wraps_origin","seed_n_variants","expanded_n_variants","median_af","af_min","af_max","af_span","observed_max","critical_count","empirical_p","sample_numt_evidence","species_numt_evidence","any_numt_evidence","numt_scope","numt_tier","numt_chrm_start","numt_chrm_end","numt_overlap_n","numt_overlap_fraction","species_numt_support_n","species_numt_support_samples","recurrence","best_recurrent_sample","shared_variants","shared_fraction","median_af_difference","cluster_class","filter_action","filter_reason"]
    variant_fields = ["sample","species","source_chrom","source_pos","source_ref","source_alt","source_af","source_dp","clustered","cluster_id","cluster_class","numt_scope","numt_tier","recurrence","filter_action","filter_reason"]
    sample_fields = ["sample","species","n_het","assessed","observed_max","critical_count","empirical_p","n_clusters","n_clustered_het","n_numt_clusters","n_numt_recurrent_clusters","n_numt_only_clusters","n_recurrent_only_clusters","n_unresolved_clusters","n_numt_variants_to_remove","fraction_het_removed","numt_sample","sample_filter_action"]
    removal_fields = ["sample","species","source_chrom","source_pos","source_ref","source_alt","source_af","source_dp","cluster_id","cluster_class","numt_scope","numt_tier","filter_action","filter_reason"]
    ...
    total_clusters, total_clustered = sum(class_counts.values()), sum(class_vars.values())
    cohort = []
    for cls in ("NUMT_RECURRENT","NUMT_ONLY","RECURRENT_ONLY","UNRESOLVED"):
        nc, nv = class_counts[cls], class_vars[cls]
        cohort.append({"cluster_class": cls, "n_clusters": nc, "fraction_clusters": f"{nc/total_clusters:.6g}" if total_clusters else "0", "n_clustered_het": nv, "fraction_clustered_het": f"{nv/total_clustered:.6g}" if total_clustered else "0"})
    write_tsv(report_dir/"heteroplasmy_analysis_summary.tsv", cohort, ["cluster_class","n_clusters","fraction_clusters","n_clustered_het","fraction_clustered_het"])
    diagnostics = [{"metadata_path": str(metadata_path), "eligible_samples": len(eligible_samples), "samples_with_species": len(eligible_samples)-len(missing_species), "samples_missing_species": len(missing_species), "numt_besthit_dir": str(besthit_dir), "samples_with_besthit_file": n_besthit_files, "numt_highconf_dir": str(highconf_dir), "samples_with_highconf_file": n_highconf_files, "parsed_numt_intervals": len(all_numts), "species_numt_intervals": len(species_rows), "detected_clusters": total_clusters}]
    write_tsv(report_dir/"heteroplasmy_input_diagnostics.tsv", diagnostics, ["metadata_path","eligible_samples","samples_with_species","samples_missing_species","numt_besthit_dir","samples_with_besthit_file","numt_highconf_dir","samples_with_highconf_file","parsed_numt_intervals","species_numt_intervals","detected_clusters"])
    return 0
