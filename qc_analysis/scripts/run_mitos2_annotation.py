...
from qc_analysis.scripts.build_mitos2_rrna_structure_table import (
 RRNA_REGION_FIELDS, RRNA_STRUCTURE_FIELDS, TRNA_STRUCTURE_FIELDS,
 build_reference_rrna_region_rows, build_reference_rrna_structure_rows,
...
def collect_reference(ref,linked,paths,settings):
 ...
 if ref.get('status') == 'skipped_no_chrM_reference':
  summary={**ref,'status':'skipped_no_chrM_reference','production_qc_status':'FAIL_PRODUCTION','production_qc_reasons':'coordinate_fasta_unavailable','command_mode':'not_run','mitos2_command':'','attempted_commands':'','return_code':'','stdout_log':'','stderr_log':'','help_log':'','n_features':0,'n_cds_features':0,'n_linked_samples':len(linked),'n_reference_coding_position_rows':0,'n_sample_level_coding_position_rows':0,'n_coding_position_rows':0,'n_reference_trna_structure_rows':0,'n_mitos2_trna_features':0,'trna_structure_status':'skipped_no_chrM_reference','trna_structure_note':'No coordinate FASTA is available for tRNA structure extraction.','n_reference_rrna_region_rows':0,'n_reference_rrna_structure_rows':0,'rrna_structure_status':'skipped_no_chrM_reference','rrna_structure_note':'No coordinate FASTA is available for rRNA structure extraction.','n_output_files_scanned':0,'n_parseable_files':0,'result_gff_exists':False,'n_gff_gene_rows':0,'n_gff_cds_like_gene_rows':0,'n_gff_trna_rows':0,'n_gff_rrna_rows':0,'parser_status':'skipped_no_chrM_reference','note':'Manifest row has no materialized chrM FASTA; accession is not a production fallback.'}
  return {'features':[],'reference_codon_rows':[],'reference_trna_structure_rows':[],'reference_rrna_region_rows':[],'reference_rrna_structure_rows':[],'summary_row':summary,'status':'skipped_no_chrM_reference','note':summary['note']}
 gff=gff_diagnostics(raw); features=[];diag=[];reference_codon_rows=[];reference_trna_structure_rows=[];reference_rrna_region_rows=[];reference_rrna_structure_rows=[];trna_structure_status='not_run';trna_structure_note='';rrna_structure_status='not_run';rrna_structure_note='';note=''
 ...
 reference_rrna_region_rows=build_reference_rrna_region_rows(ref,features)
 try:
  reference_trna_structure_rows,trna_structure_status,trna_structure_note=build_reference_trna_structure_rows(ref,features,ref.get('coordinate_reference_fasta',''),raw)
 except Exception as exc:
  trna_structure_status='failed_trna_structure_extraction';trna_structure_note=f'{type(exc).__name__}: {exc}'
 try:
...
def merge(paths,settings,refs):
 ...
 trna_structure_table=paths.get('mitos2_reference_trna_structure_table',str(Path(paths['output_dir'])/'all_mitos2_reference_trna_structure.tsv'))
 rrna_region_table=paths.get('mitos2_reference_rrna_region_table',str(Path(paths['output_dir'])/'all_mitos2_reference_rrna_regions.tsv'))
 ...
 coordinate_mapping_table=paths.get('mitos2_sample_coordinate_reference_map',str(Path(paths['output_dir'])/'sample_coordinate_reference_map.tsv'))
 outputs=[(reference_table,REFERENCE_CODON_FIELDS),(mapping_table,SAMPLE_REFERENCE_FIELDS),
          (coordinate_mapping_table,SAMPLE_REFERENCE_FIELDS),
          (trna_structure_table,TRNA_STRUCTURE_FIELDS),(rrna_region_table,RRNA_REGION_FIELDS),
 ...
 counts={'references':0,'codons':0,'trna_structures':0,'rrna_regions':0,'rrna_structures':0,'mappings':0,'features':0}; codon_mapping_rows=[]
 with ExitStack() as stack:
  ...
  for ref,linked in refs:
   ...
   trna_rows=result.get('reference_trna_structure_rows',[])
   writers[trna_structure_table].writerows(trna_rows);counts['trna_structures']+=len(trna_rows)
   rrna_region_rows=result.get('reference_rrna_region_rows',[])
   writers[rrna_region_table].writerows(rrna_region_rows);counts['rrna_regions']+=len(rrna_region_rows)
  ...
  writers[coordinate_mapping_table].writerows(validated_assignments)
 print(f"Wrote {counts['codons']} reference coding rows, {counts['trna_structures']} reference tRNA structure rows, {counts['rrna_regions']} reference rRNA regions, {counts['rrna_structures']} reference rRNA structure rows, and {counts['mappings']} production codon sample/reference mappings from {counts['references']} references.")
