...
def reset_managed_outputs(out):
    """Remove derived final-filter products without deleting Slurm manifests/logs."""
    ...
    for name in MANAGED_OUTPUT_DIRS:
        ...
        if path.is_symlink() or path.is_file():
            ...
            shutil.rmtree(path)
    for name in ("reports", "final_vcf", "final_cov", "final_mtcn", ".work"):
...
def main():
    ...
    defaults = {
        "intraspecies": (
            "results/qc/intraspecies_contamination/reports/intraspecies_contamination_report.tsv",
            ["contamination_status", "qc_status"],
        ),
        "human": (
            "results/qc/human_contamination/reports/human_contamination_report.tsv",
            ["human_contamination_status", "qc_status", "status"],
        ),
        "interspecies": (
            "results/qc/interspecies_contamination/reports/interspecies_contamination_report.tsv",
            ["interspecies_status", "qc_status", "status"],
        ),
        "sample_qc": (
                    ...
                    with open_vcf(src) as inp:
                        for line in inp:
                            if line.startswith("#"):
                                target.write(line)
                                continue
                            fields_vcf = line.rstrip("\n").split("\t")
                            key = (fields_vcf[0], fields_vcf[1], fields_vcf[3], fields_vcf[4])
                            why = list(variant_flags.get(key, []))
                            vcf_filter = fields_vcf[6]
                            if vcf_filter != "PASS":
                                why.append("vcf_filter:" + vcf_filter)
                            info, af, dp = variant_evidence(fields_vcf)
                            variant_class, snv_type = variant_classes(fields_vcf[3], fields_vcf[4])
                            ref, alt = fields_vcf[3].upper(), fields_vcf[4].upper()
                            is_canonical_snv = (
                                len(ref) == len(alt) == 1
                                and ref in "ACGT"
                                and alt in "ACGT"
                            ...
                            variant_row = dict(
                                ...
                                human_chrom=fields_vcf[0],
                                human_pos=fields_vcf[1],
                                human_ref=fields_vcf[3],
                                human_alt=fields_vcf[4],
                            ...
                            variant_writer.writerow(variant_row)
                            n_variants += 1
