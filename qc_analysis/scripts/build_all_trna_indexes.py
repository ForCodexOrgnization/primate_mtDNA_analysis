...
def add_coordinate_fastas(result, samples, min_length=14000, max_length=19000):
    ...
    for key,rows in grouped.items():
        ...
        for path in paths:
            ...
            info=normalized_fasta_sequence_sha256(path); length=info["sequence_length"]
            if not min_length <= length <= max_length:
                raise ValueError(f"Reference {key}: mitochondrial length {length} outside [{min_length}, {max_length}]: {path}")
            hashes[path]=info["sequence_sha256"]; lengths[path]=length
        observed=set(hashes.values())
        if len(observed) != 1:
            raise ValueError(f"Reference {key}: FAIL_HASH_CONFLICT: coordinate FASTAs have different normalized sequence hashes: {hashes}")
        actual=next(iter(observed)); encoded=key[6:].lower() if key.startswith("mtref_") and len(key)==70 else None
        ...
        declared={(row.get("coordinate_reference_sequence_sha256") or "").strip().lower() for row in rows}; declared.discard("")
        if len(declared)>1: raise ValueError(f"Reference {key}: conflicting coordinate_reference_sequence_sha256 values")
        ...
        canonical=paths[0]
        result[key]={"path":canonical,"target_sequence_id":None,"canonical_fasta":canonical,"n_fasta_aliases":len(paths),
                     "fasta_aliases":";".join(paths),"normalized_sequence_sha256":actual,"sequence_length":lengths[canonical],
                     "validation_status":("VALID_SINGLE_FASTA" if len(paths)==1 else "VALID_IDENTICAL_FASTA_ALIASES")}
    return result

def valid(path, reference_key):
    p=Path(path)
    if not p.is_file() or p.stat().st_size==0:return False
    try: validate_trna_index(p, reference_key); return True
    except (OSError, EOFError, ValueError):return False

def trna_chrom_normalization(reference_key, settings):
    key="human_trna_chrom_norm" if reference_key=="human" else "species_trna_chrom_norm"; configured=settings.get(key); value=str(configured or "none").strip().lower()
    if value not in SUPPORTED_CHROM_NORMALIZATIONS: raise ValueError(f"Unsupported chromosome normalization for {key}: {configured!r}")
    return value
...
def positive_integer(value,name):
    ...
    except (TypeError,ValueError): raise ValueError(f"{name} must be an integer >= 1, got {value!r}") from None
    if parsed<1: raise ValueError(f"{name} must be >= 1, got {parsed}")
    return parsed

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--config",default="config/qc_preprocessing.yaml");ap.add_argument("--workers",type=int);ap.add_argument("--overwrite",action="store_true");ap.add_argument("--reference-key");ap.add_argument("--task-manifest");a=ap.parse_args()
    cfg=read_simple_yaml(Path(a.config));sec=cfg["trna_match"];p,s=sec["paths"],sec["settings"]
    workers=positive_integer(a.workers if a.workers is not None else s.get("index_build_workers",1),"index_build_workers");trnascan_threads=positive_integer(s.get("trnascan_threads",1),"trnascan_threads")
    all_samples=table(p["sample_reference_map"])
    # An explicitly blank coordinate FASTA represents a sample/reference with no
    # materialized chrM and is an annotation skip, not a global index-build error.
    skipped_samples=[row for row in all_samples if not (row.get("coordinate_reference_fasta") or "").strip()]
    samples=[row for row in all_samples if (row.get("coordinate_reference_fasta") or "").strip()]
    minimum=int(s.get("min_mitochondrial_reference_length",14000));maximum=int(s.get("max_mitochondrial_reference_length",19000))
    fastas={}
    if p.get("reference_fasta_manifest"):fastas.update(resolve_fastas(p["reference_fasta_manifest"],minimum,maximum))
    ...
    outdir=Path(p["reference_trna_index_dir"]);reportdir=Path(p["index_build_reports_dir"]);scan=Path(p["trnascan_output_dir"]);template=p["reference_trna_index_template"]
    tasks=[];human_key="human";human_fasta=p["human_fasta"];human_output=p["human_trna_index"]
    ...
    print(f"[build_trna_indexes] n_unique_references={len(tasks)}",file=sys.stderr);print(f"[build_trna_indexes] skipped_no_coordinate_samples={len(skipped_samples)}",file=sys.stderr)
    print(f"[build_trna_indexes] index_build_workers={workers}",file=sys.stderr);print(f"[build_trna_indexes] trnascan_threads={trnascan_threads}",file=sys.stderr);print(f"[build_trna_indexes] estimated_max_cpu_usage={estimated_cpu}",file=sys.stderr)
    ...
    if allocated:
        try:allocated_cpus=positive_integer(allocated,"SLURM_CPUS_PER_TASK")
            ...
            if estimated_cpu>allocated_cpus:print(f"WARNING: estimated_max_cpu_usage={estimated_cpu} exceeds SLURM_CPUS_PER_TASK={allocated_cpus}",file=sys.stderr)
    if a.task_manifest:
        write(a.task_manifest,[{"task_id":i,"reference_key":k,"fasta":(f or {}).get("path",""),"canonical_fasta":(f or {}).get("canonical_fasta",(f or {}).get("path","")),"n_fasta_aliases":(f or {}).get("n_fasta_aliases",1 if f else 0),"fasta_aliases":(f or {}).get("fasta_aliases",(f or {}).get("path","")),"normalized_sequence_sha256":(f or {}).get("normalized_sequence_sha256",""),"sequence_length":(f or {}).get("sequence_length",""),"validation_status":(f or {}).get("validation_status",""),"output_index":o} for i,(k,f,o) in enumerate(tasks,1)],["task_id","reference_key","fasta","canonical_fasta","n_fasta_aliases","fasta_aliases","normalized_sequence_sha256","sequence_length","validation_status","output_index"]);return
    def one(task):
        ...
        provenance={name:fasta_info.get(name,"") for name in ("canonical_fasta","n_fasta_aliases","fasta_aliases","normalized_sequence_sha256","sequence_length","validation_status")}
        ...
        ns=SimpleNamespace(reference_key=key,fasta=fasta,trnascan_out=None,trnascan_ss=None,run_trnascan=True,trnascan_bin=s.get("trnascan_bin","tRNAscan-SE"),trnascan_mode=s.get("trnascan_mode","mito_mammal"),threads=trnascan_threads,trnascan_extra_args=s.get("trnascan_extra_args","") or "",trnascan_prefix=str(scan/key),output=output,overwrite=True,summary=None,chrom_normalization=trna_chrom_normalization(key,s),target_sequence_id=fasta_info.get("target_sequence_id"),allow_ss_order_fallback=bool(s.get("allow_ss_order_fallback",False)),max_sequence_mismatch_rate=s.get("max_sequence_mismatch_rate",0.0))
    ...
    for row in skipped_samples:
        maps.append({"sample":row.get("sample",row.get("sample_name","")),"reference_key":row.get("reference_key",""),"trna_index":"","status":"skipped_no_coordinate_reference","notes":"coordinate_reference_fasta is blank"})
