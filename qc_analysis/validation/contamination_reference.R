...
calibration_status <- "not_calibrated_no_file"; calibration <- data.frame()
if(!is.null(nc)) {
 if(!file.exists(nc)) stop("negative-control pair file does not exist: ",nc)
 ...
 # on an empty vector.
 calibration <- data.frame(target_negative_control_tier=P$target_negative_control_tier,n_pairs=nrow(tier2),p95_normalized_mirror=NA_real_,p99_normalized_mirror=NA_real_,mirror_calibration_status=calibration_status)
 write_tsv(calibration,"tier2_normalized_mirror_threshold_summary.tsv")
}
d <- read.delim(vt,check.names=FALSE,stringsAsFactors=FALSE,na.strings=c("","NA",".")); req <- c("Sample","Species","CHROM","POS","REF","ALT","Type","FILTER","DP")
if(!all(req %in% names(d)) || !any(c("VAF","AF") %in% names(d))) stop("variant table lacks required columns")
if(!"VAF" %in% names(d)) d$VAF <- d$AF
if(any(!nzchar(d$Sample)|!nzchar(d$Species))||any(!is.finite(suppressWarnings(as.numeric(d$DP))))||any(!is.finite(suppressWarnings(as.numeric(d$VAF)))|d$VAF<0|d$VAF>1)||any(!is.finite(suppressWarnings(as.numeric(d$POS)))|d$POS<1|d$POS!=floor(d$POS))) stop("invalid Sample, Species, DP, VAF, or POS")
key <- do.call(paste,c(d[c("Species","Sample","CHROM","POS","REF","ALT")],sep="\r")); dup <- duplicated(key)
