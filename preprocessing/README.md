...
variant_reference_slurm_time: "24:00:00"
variant_reference_slurm_cpus: 4
variant_reference_slurm_mem: "16G"
curl_command: "curl"
...
When Slurm `sbatch` is available, `variant_references` submits one array task per species/reference package and uses `variant_reference_threads` as the `%N` array concurrency limit. Each array task requests `variant_reference_slurm_time` (default `24:00:00`), `variant_reference_slurm_cpus` (default `4`), and `variant_reference_slurm_mem` (default `16G`). For example, `variant_reference_threads: 100` submits up to 100 reference packages at the same time, followed by a dependent merge job that combines shard manifests into `variant_calling_reference_manifest.tsv`. If `sbatch` is unavailable or `RUN_LOCAL=1`, the same thread setting controls local background worker concurrency instead.

Set `variant_reference_skip_existing: true` in `config/preprocessing_paths.yaml` (or `VARIANT_REFERENCE_SKIP_EXISTING=1`) to skip a species/reference package when its expected output FASTA files and indexes already exist in the output directory. Use `variant_reference_force: true` (or `VARIANT_REFERENCE_FORCE=1`) to rebuild even when existing outputs are present.

The generated reference package includes the whole-genome FASTA with configured NUMT masking applied where eligible, a nuclear-only FASTA under `references/variant_calling/nuclear_only_refs/` with chrM/mitochondrial contigs removed, the chrM FASTA, shifted chrM FASTA, FASTA indexes, sequence dictionaries, BWA indexes, interval files, shift-back chain files, and `variant_calling_reference_manifest.tsv`.
