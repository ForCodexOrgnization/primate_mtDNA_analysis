...
require_sample_metadata() {
  local path="$1"
  if [[ ! -s "$path" ]]; then
    cat >&2 <<EOFMSG
ERROR: missing or empty sample metadata: ${path}
This is a user-provided sample sheet, not a file generated by the preprocessing pipeline.
Create it from the template before running the variant_inputs or post_reference_review steps:
  cp data/metadata/sample_metadata.template.tsv ${path}
Then add one row per sequencing sample with required columns:
  sample_id, target_species, cram_path, cram_index_path
EOFMSG
    exit 1
  fi

  local header
  IFS= read -r header < "$path" || header=""
  for required_col in sample_id target_species cram_path cram_index_path; do
    if ! printf '%s\t' "$header" | tr '\t' '\n' | grep -Fxq "$required_col"; then
      echo "ERROR: sample metadata is missing required column '${required_col}': ${path}" >&2
      echo "Expected at least: sample_id, target_species, cram_path, cram_index_path" >&2
      exit 1
    fi
  done

  local data_rows
  data_rows=$(awk 'NR > 1 && $0 !~ /^[[:space:]]*$/ { count++ } END { print count + 0 }' "$path")
  if [[ "$data_rows" -eq 0 ]]; then
    cat >&2 <<EOFMSG
ERROR: sample metadata has no sample rows: ${path}
This user-provided file must contain one row per sequencing sample after the header.
Required columns are: sample_id, target_species, cram_path, cram_index_path
EOFMSG
    exit 1
  fi
}
...
run_variant_inputs() {
  ...
  require_sample_metadata "$SAMPLE_METADATA"
