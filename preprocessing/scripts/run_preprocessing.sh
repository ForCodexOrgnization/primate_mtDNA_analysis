...
VARIANT_REFERENCE_SKIP_EXISTING=$(config_get variant_reference_skip_existing "0")
...
run_variant_references() {
  ...
  echo "[preprocessing] VARIANT_REFERENCE_SKIP_EXISTING=${VARIANT_REFERENCE_SKIP_EXISTING}" >&2
  ...
  VARIANT_REFERENCE_SKIP_EXISTING="$VARIANT_REFERENCE_SKIP_EXISTING" \
  PYTHON_COMMAND="$PYTHON_COMMAND" \
  SAMTOOLS_COMMAND="$SAMTOOLS_COMMAND" \
