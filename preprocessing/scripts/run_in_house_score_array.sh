...
if [[ "${MERGE_ONLY:-0}" == "1" ]]; then
  REF_INPUTS="$REF_INPUTS" OUTDIR="$OUTDIR" MERGED_IN_HOUSE_SCORE="$MERGED_IN_HOUSE_SCORE" MERGE_ONLY=1 bash "$IN_HOUSE_SCORE_SCRIPT"
...
if [[ "${RUN_LOCAL:-0}" != "1" ]] && command -v sbatch >/dev/null 2>&1; then
  jid=$(sbatch --parsable --array="1-${N}%${MAX_CONCURRENT}" --export=ALL,REF_INPUTS="$REF_INPUTS",OUTDIR="$OUTDIR",MERGED_IN_HOUSE_SCORE="$MERGED_IN_HOUSE_SCORE" "$IN_HOUSE_SCORE_SCRIPT")
  echo "Submitted in-house score array job ${jid} for ${N} reference rows." >&2
  merge_jid=$(sbatch --parsable --dependency="afterok:${jid}" --export=ALL,REF_INPUTS="$REF_INPUTS",OUTDIR="$OUTDIR",MERGED_IN_HOUSE_SCORE="$MERGED_IN_HOUSE_SCORE",MERGE_ONLY=1 "$IN_HOUSE_SCORE_SCRIPT")
  ...
  for i in $(seq 1 "$N"); do
    SLURM_ARRAY_TASK_ID="$i" REF_INPUTS="$REF_INPUTS" OUTDIR="$OUTDIR" bash "$IN_HOUSE_SCORE_SCRIPT"
  done
  REF_INPUTS="$REF_INPUTS" OUTDIR="$OUTDIR" MERGED_IN_HOUSE_SCORE="$MERGED_IN_HOUSE_SCORE" MERGE_ONLY=1 bash "$IN_HOUSE_SCORE_SCRIPT"
  bash preprocessing/scripts/merge_in_house_score.sh "$OUTDIR" "$MERGED_IN_HOUSE_SCORE"
fi
